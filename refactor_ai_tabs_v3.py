    ...
    while i < len(lines):
        ...
        if "today_str = date.today().strftime('%d%m%y')" in line and not in_ai_block:
            ...
            while i < len(lines):
                ...
                if curr_stripped.startswith("if ai_cache_file.exists():"):
                    seen_if = True
                elif curr_stripped.startswith("else:"):
                    seen_else = True
...
{indent_str}            st.info("ℹ️ Không thể đọc được danh sách lịch sử.")\n"""
                ...
                # Handling github upload block removal
