...
AI_CIO_DETERMINISTIC_PROVIDER = "quant-rules-v1"
AI_CIO_HISTORY_PROVIDERS = frozenset({
    AI_CIO_HISTORY_PROVIDER,
...
def upsert_history_csv(
    ...
    """
    if provider not in AI_CIO_HISTORY_PROVIDERS:
        print(
            f"[CSV] Skip history upsert: provider={provider} is not a history provider."
        )
        return False
...
def _build_ai_cio_metrics_snapshot(
    ...
    return {
        ...
        "data_date": data_date,
        "authority_rules": [
            "This JSON is generated deterministically before report rendering.",
            ...
            "Any optional narrative must not relabel adapter outputs from prose.",
...
def _read_recent_summary_ledger(provider_key: str = "kimi-2.6", n_past: int = AI_CIO_HISTORY_WINDOW) -> list[dict[str, Any]]:
    ...
    compatible_providers = (
        AI_CIO_HISTORY_PROVIDERS
        if provider_key == AI_CIO_DETERMINISTIC_PROVIDER
        else frozenset({provider_key})
    )
    provider_rows = [
        row for row in rows if row.get("provider") in compatible_providers or not row.get("provider")
    ]
...
def _render_rule_based_executive_report(
    decision_state: dict[str, Any],
    metrics_snapshot: dict[str, Any],
    report_date: str,
    data_date: str,
) -> str:
    """Build the daily report from audited Python outputs without an LLM."""

    score = _safe_float(decision_state.get("metric_implied_score"))
    if score is None:
        raise ValueError("AI CIO deterministic score is unavailable")
    stress_regime = regime_from_score(score)
    resolved_regime = (
        "CAPITULATION" if _capitulation_action_eligible(decision_state) else stress_regime
    )
    capitulation = decision_state.get("capitulation_state") or {}
    subscores = decision_state.get("metric_implied_subscores") or {}
    history = (metrics_snapshot.get("history") or {}).get("rolling_summary") or {}
    constraints = decision_state.get("hard_constraints") or []

    def cell(value: Any) -> str:
        if value is None or value == "":
            return "N/A"
        return re.sub(r"\s+", " ", str(value)).replace("|", "/")[:120]

    lines = [
        "# AI CIO Daily Quant Report",
        "",
        f"Report date: {report_date} | Market data: {data_date}",
        "",
        "This report is generated from quantitative evidence and fixed policy rules. "
        "Scores and limits come from the metrics snapshot; missing inputs are not estimated.",
        "",
        "### 1. Composite score and regime",
        f"- Composite score: {score:g}/100",
        f"- Stress regime: {stress_regime}",
        f"- Resolved regime: {resolved_regime}",
        f"- Capitulation phase: {cell(capitulation.get('phase'))}",
        f"- Capitulation action eligible: {capitulation.get('action_eligible') is True}",
    ]
    delta = _safe_float(history.get("score_change_1d"))
    if delta is not None:
        lines.append(f"- Score change versus prior session: {delta:+g}")

    lines.extend(["", "### 2. Score components", "| Component | Score |", "| --- | ---: |"])
    for key, label in (
        ("macro_risk_score", "Macro risk"),
        ("market_internal_score", "Market internals"),
        ("tail_risk_score", "Tail risk"),
    ):
        component = _safe_float(subscores.get(key))
        lines.append(f"| {label} | {component:g} |" if component is not None else f"| {label} | DATA INSUFFICIENT |")

    lines.extend([
        "",
        "### 3. Tool evidence",
        "| Tool | As of | Score | Regime | Metric source |",
        "| --- | --- | ---: | --- | --- |",
    ])
    tool_count = 0
    for tool_id, tool in (metrics_snapshot.get("tools") or {}).items():
        if not isinstance(tool, dict) or tool.get("scoring_eligible") is False:
            continue
        tool_count += 1
        tool_score = _safe_float(tool.get("tool_score"))
        score_text = f"{tool_score:g}" if tool_score is not None else "N/A"
        lines.append(
            f"| {cell(tool_id)} | {cell(tool.get('as_of'))} | {score_text} | "
            f"{cell(tool.get('tool_regime'))} | {cell(tool.get('data_quality'))} |"
        )
    if not tool_count:
        lines.append("| DATA INSUFFICIENT | N/A | N/A | N/A | N/A |")

    lines.extend(["", "### 4. Hard constraints"])
    lines.extend(f"- {cell(item)}" for item in constraints)
    if not constraints:
        lines.append("- No hard constraint was raised by the current snapshot.")
    lines.extend([
        "",
        "### 5. Audit trail",
        "- Composite score, tool metrics, freshness, and policy details are in the daily AI CIO metrics JSON.",
        "- Tool scores marked N/A are not inferred from narrative text.",
    ])
    return "\n".join(lines)
...
def run_executive_summary(api_key: str, provider_key: str = "kimi-2.6", force: bool = False,
                          source: str = "manual", report_mode: str = "llm"):
    if report_mode not in {"llm", "deterministic"}:
        raise ValueError(f"Unsupported AI CIO report mode: {report_mode}")
    if report_mode == "deterministic" and provider_key != AI_CIO_DETERMINISTIC_PROVIDER:
        raise ValueError("Deterministic AI CIO reports require the quant-rules-v1 provider")
    if report_mode == "llm" and provider_key == AI_CIO_DETERMINISTIC_PROVIDER:
        raise ValueError("The quant-rules-v1 provider cannot call a language model")
    cfg = AI_PROVIDER_MAP.get(provider_key, AI_PROVIDER_MAP["kimi-2.6"]) if report_mode == "llm" else None
    final_client = (
        OpenAI(api_key=api_key.strip(), base_url=cfg["base_url"], timeout=cfg.get("timeout", 180))
        if cfg is not None else None
    )
    child_client = _DeterministicChildClient()
    model = cfg["api_model"] if cfg is not None else AI_CIO_DETERMINISTIC_PROVIDER
    temperature = cfg.get("temperature", 1.0) if cfg is not None else None
    ...
    if report_mode == "deterministic":
        raw_final_res = _render_rule_based_executive_report(
            decision_state, metrics_snapshot, report_date, data_date
        )
    else:
    ...
    if report_mode == "llm":
        final_res, _ = postprocess_executive_summary_report(
            raw_final_res,
            provider_key,
            decision_state=decision_state,
        )
    else:
        final_res = raw_final_res
