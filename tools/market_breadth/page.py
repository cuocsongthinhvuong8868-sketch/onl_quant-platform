...
def render():
    ...
    with tab_current:
        if ai_cache_file.exists():
            ...
            render_sync_button(ai_cache_file, key_suffix="market_breadth")
    ...
    with tab_history:
        ...
        _all_caches = list(DATA_LAKE.glob("daily_cache/market_breadth_*.txt"))
        _options = build_history_options(_all_caches, "market_breadth", AI_PROVIDER_MAP)
        if not _options:
            ...
            _selected_label = st.selectbox(
                ...
                index=0,
                key="market_breadth_history_selector"
            )
